...
def promote(version, channel, path, subject, body, github_repo, github_token, upstream_remote='origin', upstream_branch='master'):
    if not github_token:
        raise ValueError('cannot promote without a configured GitHub token')
    subprocess.run(['git', 'fetch', upstream_remote], check=True)
    branch = 'promote-{}-to-{}'.format(version, channel)
    ...
    if version in versions:
        raise ValueError('version {} has already been promoted to {} in {}/{}'.format(version, channel, upstream_remote, upstream_branch))
    versions.add(version)
    data['versions'] = list(sorted(versions, key=semver_sort_key))
    with open(path, 'w') as f:
        yaml.safe_dump(data, f, default_flow_style=False)
    message = '{}\n\n{}\n'.format(subject, textwrap.fill(body, width=76))
    subprocess.run(['git', 'commit', '--file', '-', path], check=True, encoding='utf-8', input=message)
